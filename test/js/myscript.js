...
const animItems = document.querySelectorAll('._animaciya');
    ...
    $('#forma1').submit(function(event){
        ...
        $.ajax({
            ...
            document.getElementById("uspokoenie").innerHTML = "Заявка успешно отправлена! Теперь вы можете закрыть это меню.";
            $("form").trigger("reset");
        });
        return false;
    });
...
window.addEventListener('load', () => {
   mask.classList.add('hide_preloader'); 
    setTimeout(() => { 
        mask.remove();
    }, 600);
});
